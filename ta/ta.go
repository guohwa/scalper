...
func Hma(inReal []float64, inTimePeriod int) []float64 {
	wma0 := Wma(inReal, inTimePeriod/2)
	wma1 := Wma(inReal, inTimePeriod)

	double := make([]float64, len(inReal))
	Fill(double, 2.0)

	wma2 := Mult(wma0, double)
	wma3 := Sub(wma2, wma1)

	timePeriod := int(math.Floor(math.Sqrt(float64(inTimePeriod))))

	return Wma(wma3, timePeriod)
}
...
func Fill(inReal []float64, value float64) {
	for i := 0; i < len(inReal); i++ {
		inReal[i] = value
	}
}
